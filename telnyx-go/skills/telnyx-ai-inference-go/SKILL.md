...
`client.SpeechToText.Transcribe()` — `GET /speech-to-text/transcription`

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `Language` | string | No | The language spoken in the audio stream. |
| `InterimResults` | boolean | No | Whether to receive interim transcription results. |
| `Model` | object | No | The specific model to use within the selected transcription ... |
| ... | | | +4 optional params in [references/api-details.md](references/api-details.md) |

```go
	err := client.SpeechToText.Transcribe(context.Background(), telnyx.SpeechToTextTranscribeParams{
...
`client.TextToSpeech.Stream()` — `GET /text-to-speech/speech`

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `Provider` | enum (aws, telnyx, azure, elevenlabs, minimax, ...) | No | TTS provider. |
| `ModelId` | string (UUID) | No | Model identifier for the chosen provider. |
| `VoiceId` | string (UUID) | No | Voice identifier for the chosen provider. |
| ... | | | +4 optional params in [references/api-details.md](references/api-details.md) |

```go
	err := client.TextToSpeech.Stream(context.Background(), telnyx.TextToSpeechStreamParams{})
	if err != nil {
		log.Fatal(err)
	}
```
